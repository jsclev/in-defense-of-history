...
final class SolidShotFlightTests: XCTestCase {
    func testLongFrameHitsEveryBodyInTravelOrderButNotOffLine() {
        var shot = SolidShotFlight(range: 400, hitRadius: AuthoredDatabaseFixture.combatRules.solidShotHitRadius)
        ...
        let hits = shot.contacts(from: .zero, to: end, targets: [
            ...
            (2, CGPoint(x: 140, y: AuthoredDatabaseFixture.combatRules.solidShotHitRadius)),
            (4, CGPoint(x: 200, y: AuthoredDatabaseFixture.combatRules.solidShotHitRadius + 1))])
    ...
    func testTravelEndsAtRangeInEveryDirection() {
        for degrees in stride(from: 0, to: 360, by: 15) {
            ...
            var shot = SolidShotFlight(range: 478.63, hitRadius: AuthoredDatabaseFixture.combatRules.solidShotHitRadius)
    ...
    func testStationaryPausedOrInvalidTimeCannotConsumeFlight() {
        var shot = SolidShotFlight(range: 100, hitRadius: AuthoredDatabaseFixture.combatRules.solidShotHitRadius)
