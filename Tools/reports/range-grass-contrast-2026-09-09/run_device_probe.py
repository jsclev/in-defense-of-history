...
BUNDLE_ID = 'com.zippyzen.range-grass-probe'
RUN_ID = str(uuid.uuid4())

def run(*args, **kwargs):
    return subprocess.run(args, check=True, **kwargs)

with tempfile.TemporaryDirectory(prefix='td-range-probe-') as temp_name:
    temp = Path(temp_name)
    root = temp / GAME.name
    root.mkdir()
    ...
    helpers += '\n' + (REPORT / 'before.swift').read_text().replace('TowerRangeOverlayView', 'BeforeTowerRangeOverlayView')
    (root / 'Liberty Line/LibertyLineApp.swift').write_text((REPORT / 'RangeProbe.swift').read_text().replace('PROBE_RUN_ID', RUN_ID) + '\n' + helpers)
    ...
    (REPORT / 'tested-map-sha256.txt').write_text(hashlib.sha256((GAME.parent / 'in-defense-of-history-data/Levels/level_01_battle_road.heic').read_bytes()).hexdigest()+'\n')
    build = temp / 'build'
    with (REPORT / 'probe-build.log').open('w') as log:
        run('xcodebuild', '-project', str(root / 'InDefenseOfHistory.xcodeproj'), '-scheme', 'Liberty Line',
            ...
            'INFOPLIST_KEY_CFBundleDisplayName=Range Art Check', 'build', stdout=log, stderr=subprocess.STDOUT)
    ...
    result = REPORT / 'range-check.json'
    deadline = time.monotonic() + 240
    while time.monotonic() < deadline:
        copied = subprocess.run(['xcrun', 'devicectl', 'device', 'copy', 'from', '--device', DEVICE,
            '--domain-type', 'appDataContainer', '--domain-identifier', BUNDLE_ID,
            '--source', 'Documents/range-check.json', '--destination', str(result)], capture_output=True)
    ...
    print("PASS: physical-iPhone range overlay captures saved on the new grass.")
