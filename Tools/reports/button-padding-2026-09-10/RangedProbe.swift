        ...
        .task {
            ...
            do {
                ...
                for small in [false, true] {
                    ...
                    let icon = store.towerMenuLayout.getTowerIconSize(towerButtonSize: side, for: .ranged)
                    ...
                    try savePaddingButtons(canvas: canvas, layout: store.towerMenuLayout, mode: mode, directory: directory)
...
@MainActor private func savePaddingButtons(canvas: RuntimeCanvas, layout: TowerMenuLayout, mode: String, directory: URL) throws {
    let tower = layout.getTowerButtonSize(playAreaScalingFactor: canvas.scaleFactor).width
    let hero = HeroBarLayout(runtimeCanvas: canvas).buttonSize
    let misc = min(canvas.miscViewSize.width, canvas.miscViewSize.height)
    let wave = HudSizing.callWaveButton.resolved(HudScale(playableHeight: canvas.playAreaRect.height))
    let views: [(String, AnyView, CGFloat)] = [
        ("gun-before", AnyView(HudButtonView(iconName: "tower_menu_ranged_square", buttonSize: tower, iconScale: 0.8, frameName: "hud_misc_sack_blue_frame") {}), tower),
        ("gun-after", AnyView(HudButtonView(iconName: "tower_menu_ranged_square", buttonSize: tower, frameName: "hud_misc_sack_blue_frame") {}), tower),
        ("sack-before", AnyView(HudButtonView(iconName: "hud_misc_sack", buttonSize: misc, iconScale: 0.8, frameName: "hud_misc_sack_blue_frame") {}), misc),
        ("sack-after", AnyView(HudMiscView(runtimeCanvas: canvas)), misc),
        ("swords-before", AnyView(HudButtonView(iconName: "action_icon_call_reinforcements", buttonSize: hero, iconScale: 0.75) {}), hero),
        ("swords-after", AnyView(ReinforcementButton(buttonSize: CGSize(width: hero, height: hero), cooldown: .ready, isAvailable: true, action: {})), hero),
        ("horn-before", AnyView(Image(CallWaveButtonLayout.imageName).resizable().scaledToFit().frame(width: wave, height: wave)), wave),
        ("horn-after", AnyView(CallWaveArtwork(side: wave)), wave)
    ]
    for (name, view, side) in views {
        for density in [CGFloat(1), 2, 3] {
            let renderer = ImageRenderer(content: view.frame(width: side, height: side))
            renderer.scale = density
            guard let bytes = renderer.uiImage?.pngData() else { throw failure("Padding render failed") }
