...
BUNDLE_ID = 'com.zippyzen.knox-regenerated-probe'
...
with tempfile.TemporaryDirectory(prefix='td-ranged-probe-') as temp_name:
    ...
    (root / 'Liberty Line/LibertyLineApp.swift').write_text((REPORT / 'KnoxHUDProbe.swift').read_text().replace('PROBE_RUN_ID', RUN_ID) + '\n' + helpers)
    ...
    with (REPORT / 'probe-build.log').open('w') as log:
        run('xcodebuild', '-project', str(root / 'InDefenseOfHistory.xcodeproj'), '-scheme', 'Liberty Line',
            ...
            'INFOPLIST_KEY_CFBundleDisplayName=Knox Regenerated Check', 'build', stdout=log, stderr=subprocess.STDOUT)
    ...
    result = REPORT / 'reinforcements-check.json'
    deadline = time.monotonic() + 240
    while time.monotonic() < deadline:
        copied = subprocess.run(['xcrun', 'devicectl', 'device', 'copy', 'from', '--device', DEVICE,
            '--domain-type', 'appDataContainer', '--domain-identifier', BUNDLE_ID,
            '--source', 'Documents/reinforcements-check.json', '--destination', str(result)], capture_output=True)
    ...
    print("PASS: production reinforcement button and live HUD captured at minimum and device sizes.")
