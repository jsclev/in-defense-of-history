"""Render the production grass terrain and gameplay views in a separate physical-iPhone app."""
import json
import hashlib
import shutil
import subprocess
...
BUNDLE_ID = 'com.zippyzen.geojson-slots-probe'
RUN_ID = str(uuid.uuid4())

def run(*args, **kwargs):
    return subprocess.run(args, check=True, **kwargs)

with tempfile.TemporaryDirectory(prefix='td-geojson-slots-probe-') as temp_name:
    temp = Path(temp_name)
    root = temp / GAME.name
    root.mkdir()
    ...
    (root / 'Liberty Line/LibertyLineApp.swift').write_text((REPORT / 'SlotProbe.swift').read_text().replace('PROBE_RUN_ID', RUN_ID) + '\n' + helpers)
    ...
    compiled = Path('/private/tmp/td-geojson-slots-build/Build/Products/Debug-iphoneos/Liberty Line.app/Assets.car')
    if compiled.exists():
    ...
    inputs = [GAME / 'Liberty Line/LevelRunner.swift', GAME / 'Engine/Persistence/LevelLoader.swift', GAME / 'Engine/Persistence/Data Access Objects/LevelGeoJSONDAO.swift', GAME / 'Db/level_15_charleston.geojson']
    inputs += list((GAME.parent / 'in-defense-of-history-data/LibertyLineAssets.xcassets/tower_slot_available.imageset').glob('*'))
    ...
    with (REPORT / 'probe-build.log').open('w') as log:
        run('xcodebuild', '-project', str(root / 'InDefenseOfHistory.xcodeproj'), '-scheme', 'Liberty Line',
            ...
            'INFOPLIST_KEY_CFBundleDisplayName=Tower Site Check', 'build', stdout=log, stderr=subprocess.STDOUT)
    ...
    result = REPORT / 'slot-check.json'
    deadline = time.monotonic() + 240
    while time.monotonic() < deadline:
        copied = subprocess.run(['xcrun', 'devicectl', 'device', 'copy', 'from', '--device', DEVICE,
            '--domain-type', 'appDataContainer', '--domain-identifier', BUNDLE_ID,
            '--source', 'Documents/slot-check.json', '--destination', str(result)], capture_output=True)
    ...
    print('PASS: all 15 maps match GeoJSON, final-slot build and phone captures; temporary probe removed.')
