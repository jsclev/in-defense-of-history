"""Render the production sack HUD icon and Charleston gameplay in a separate physical-iPhone app."""
import json
import hashlib
import shutil
import subprocess
...
BUNDLE_ID = 'com.zippyzen.misc-sack-blue-probe'
RUN_ID = str(uuid.uuid4())

def run(*args, **kwargs):
    return subprocess.run(args, check=True, **kwargs)

with tempfile.TemporaryDirectory(prefix='td-misc-sack-probe-') as temp_name:
    temp = Path(temp_name)
    root = temp / GAME.name
    root.mkdir()
    ...
    (root / 'Liberty Line/LibertyLineApp.swift').write_text((REPORT / 'MiscSackProbe.swift').read_text().replace('PROBE_RUN_ID', RUN_ID) + '\n' + helpers)
    ...
    inputs = [GAME / 'Liberty Line/Level Views/HUD/HudMiscView.swift', GAME / 'Liberty Line/Level Views/HUD/HudButtonView.swift']
    inputs += list((GAME.parent / 'in-defense-of-history-data/LibertyLineAssets.xcassets/hud_misc_sack.imageset').glob('*'))
    ...
    frameInputs = list((GAME.parent / 'in-defense-of-history-data/LibertyLineAssets.xcassets/hud_misc_sack_blue_frame.imageset').glob('*'))
    hashes = json.loads((REPORT / 'tested-source-hashes.json').read_text())
    hashes.update({str(f): hashlib.sha256(f.read_bytes()).hexdigest() for f in frameInputs})
    (REPORT / 'tested-source-hashes.json').write_text(json.dumps(hashes, indent=2))
    build = temp / 'build'
    with (REPORT / 'probe-build.log').open('w') as log:
        run('xcodebuild', '-project', str(root / 'InDefenseOfHistory.xcodeproj'), '-scheme', 'Liberty Line',
            ...
            'INFOPLIST_KEY_CFBundleDisplayName=Misc Blue Check', 'build', stdout=log, stderr=subprocess.STDOUT)
    ...
    result = REPORT / 'misc-check.json'
    deadline = time.monotonic() + 240
    while time.monotonic() < deadline:
        copied = subprocess.run(['xcrun', 'devicectl', 'device', 'copy', 'from', '--device', DEVICE,
            '--domain-type', 'appDataContainer', '--domain-identifier', BUNDLE_ID,
            '--source', 'Documents/misc-check.json', '--destination', str(result)], capture_output=True)
    ...
    print("PASS: compiled sack sprite, lower-right HUD, and physical-iPhone gameplay captures at minimum and device sizes.")
