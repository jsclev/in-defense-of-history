...
BUNDLE_ID = 'com.zippyzen.call-wave-art-probe'
...
with tempfile.TemporaryDirectory(prefix='td-misc-sack-probe-') as temp_name:
    ...
    (root / 'Liberty Line/LibertyLineApp.swift').write_text((REPORT / 'CallWaveProbe.swift').read_text().replace('PROBE_RUN_ID', RUN_ID) + '\n' + helpers)
    ...
    inputs = [GAME / 'Liberty Line/Level Views/HUD/CallWaveButtonView.swift', GAME / 'Engine/Layout/CallWaveButtonLayout.swift']
    inputs += list((GAME.parent / 'in-defense-of-history-data/LibertyLineAssets.xcassets/hud_call_wave.imageset').glob('*'))
    ...
    with (REPORT / 'probe-build.log').open('w') as log:
        run('xcodebuild', '-project', str(root / 'InDefenseOfHistory.xcodeproj'), '-scheme', 'Liberty Line',
            ...
            'INFOPLIST_KEY_CFBundleDisplayName=Call Wave Check', 'build', stdout=log, stderr=subprocess.STDOUT)
    ...
    result = REPORT / 'call-wave-check.json'
    deadline = time.monotonic() + 240
    while time.monotonic() < deadline:
        copied = subprocess.run(['xcrun', 'devicectl', 'device', 'copy', 'from', '--device', DEVICE,
            '--domain-type', 'appDataContainer', '--domain-identifier', BUNDLE_ID,
            '--source', 'Documents/call-wave-check.json', '--destination', str(result)], capture_output=True)
    ...
    print("PASS: compiled call-wave art, real wave start, native countdown, and live pulse at minimum and device sizes.")
