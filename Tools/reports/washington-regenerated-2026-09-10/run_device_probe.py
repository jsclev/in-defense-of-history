...
BUNDLE_ID = 'com.zippyzen.washington-framed-probe'
...
with tempfile.TemporaryDirectory(prefix='td-ranged-probe-') as temp_name:
    ...
    (root / 'Liberty Line/LibertyLineApp.swift').write_text((REPORT / 'WashingtonHUDProbe.swift').read_text().replace('PROBE_RUN_ID', RUN_ID) + '\n' + helpers)
    ...
    names = ['action_icon_call_reinforcements','hud_misc_sack_blue_frame','tower_menu_square_frame','hero_icon_george_washington','hero_icon_henry_knox','hud_hero_henry_knox_framed','hud_hero_george_washington_framed']
    for name in names:
    ...
    with (REPORT / 'probe-build.log').open('w') as log:
        run('xcodebuild', '-project', str(root / 'InDefenseOfHistory.xcodeproj'), '-scheme', 'Liberty Line',
            ...
            'INFOPLIST_KEY_CFBundleDisplayName=Washington Icon Check', 'build', stdout=log, stderr=subprocess.STDOUT)
