...
BUNDLE_ID = 'com.zippyzen.charleston-grass-probe'
...
with tempfile.TemporaryDirectory(prefix='td-grass-probe-') as temp_name:
    ...
    (root / 'Liberty Line/LibertyLineApp.swift').write_text((REPORT / 'GrassProbe.swift').read_text().replace('PROBE_RUN_ID', RUN_ID) + '\n' + helpers)
    ...
    (REPORT / 'tested-map-sha256.txt').write_text(hashlib.sha256((GAME.parent / 'in-defense-of-history-data/Levels/level_15_charleston.heic').read_bytes()).hexdigest()+'\n')
    build = temp / 'build'
    with (REPORT / 'probe-build.log').open('w') as log:
        run('xcodebuild', '-project', str(root / 'InDefenseOfHistory.xcodeproj'), '-scheme', 'Liberty Line',
            ...
            'INFOPLIST_KEY_CFBundleDisplayName=Grass Art Check', 'build', stdout=log, stderr=subprocess.STDOUT)
    ...
    result = REPORT / 'grass-check.json'
    deadline = time.monotonic() + 240
    while time.monotonic() < deadline:
        copied = subprocess.run(['xcrun', 'devicectl', 'device', 'copy', 'from', '--device', DEVICE,
            '--domain-type', 'appDataContainer', '--domain-identifier', BUNDLE_ID,
            '--source', 'Documents/grass-check.json', '--destination', str(result)], capture_output=True)
