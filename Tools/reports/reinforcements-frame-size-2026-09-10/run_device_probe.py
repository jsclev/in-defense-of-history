...
BUNDLE_ID = 'com.zippyzen.reinforcements-size-probe'
...
with tempfile.TemporaryDirectory(prefix='td-ranged-probe-') as temp_name:
    ...
    (root / 'Liberty Line/LibertyLineApp.swift').write_text((REPORT / 'ReinforcementsProbe.swift').read_text().replace('PROBE_RUN_ID', RUN_ID) + '\n' + helpers)
    ...
    names = ['action_icon_call_reinforcements','hud_misc_sack_blue_frame','tower_menu_square_frame','hero_icon_george_washington','hero_icon_henry_knox']
    for name in names:
    ...
    with (REPORT / 'probe-build.log').open('w') as log:
        run('xcodebuild', '-project', str(root / 'InDefenseOfHistory.xcodeproj'), '-scheme', 'Liberty Line',
            ...
            'INFOPLIST_KEY_CFBundleDisplayName=Reinforcement Size Check', 'build', stdout=log, stderr=subprocess.STDOUT)
