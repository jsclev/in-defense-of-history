...
BUNDLE_ID = 'com.zippyzen.crown-contrast-probe'
RUN_ID = str(uuid.uuid4())

def run(*args, **kwargs):
    return subprocess.run(args, check=True, **kwargs)

with tempfile.TemporaryDirectory(prefix='td-crown-probe-') as temp_name:
    temp = Path(temp_name)
    root = temp / GAME.name
    root.mkdir()
    ...
    shader = root / 'Liberty Line/Shaders.metal'
    shader_source = shader.read_text()
    assert 'return color * faceCoverage;' in shader_source
    shader_source = shader_source.replace('return color * faceCoverage;',
        'half4 face = color * faceCoverage;\n    return face * smoothstep(half(0.01), half(0.03), face.a);')
    shader.write_text(shader_source + '\n' + (REPORT / 'CrownKeyline.metal').read_text())
    appsource = root / 'Liberty Line/LibertyLineApp.swift'
    appsource.write_text(appsource.read_text() + '\n' + (REPORT / 'CrownOutlineCandidate.swift').read_text())
    view = root / 'Liberty Line/Level Views/LevelExitMarkersView.swift'
    oldview = view.read_text()
    start = oldview.index('            Image("path_exit_crown")')
    end = oldview.index('                .position(', start)
    oldview = oldview[:start] + '            CrownOutlineCandidate(outlinePoints: 0.9)\n                .scaleEffect(spriteSize / 96)\n                .frame(width: spriteSize, height: spriteSize)\n' + oldview[end:]
    view.write_text(oldview)
    # Reuse the exact catalog just compiled in our production build.
    compiled = Path('/private/tmp/td-gold-crown-device-build/Build/Products/Debug-iphoneos/Liberty Line.app/Assets.car')
    assert compiled.is_file(), 'Production crown build must finish first'
    shutil.copy2(compiled, root / 'Assets.car')
    project = root / 'InDefenseOfHistory.xcodeproj/project.pbxproj'
    text = project.read_text()
    old = 'lastKnownFileType = folder.assetcatalog; name = LibertyLineAssets.xcassets; path = "../in-defense-of-history-data/LibertyLineAssets.xcassets";'
    assert old in text
    project.write_text(text.replace(old, 'lastKnownFileType = file; name = Assets.car; path = Assets.car;'))
    import hashlib
    (REPORT / 'compiled-catalog.json').write_text(json.dumps({'path': str(compiled), 'sha256': hashlib.sha256(compiled.read_bytes()).hexdigest()}, indent=2))
    build = temp / 'build' 
    with (REPORT / 'probe-build.log').open('w') as log:
        run('xcodebuild', '-project', str(root / 'InDefenseOfHistory.xcodeproj'), '-scheme', 'Liberty Line',
            ...
            'INFOPLIST_KEY_CFBundleDisplayName=Crown Contrast Check', 'build', stdout=log, stderr=subprocess.STDOUT)
