...
@MainActor private func checkAllExits(store: Store, directory: URL) throws -> [[String: Any]] {
    ...
    try require(UIImage(named: "path_exit_crown") != nil, "Approved Gold Stamp crown missing from compiled catalog")
    let crown = UIImage(named: "path_exit_crown")!
    try require(crown.size == CGSize(width: 96, height: 96), "Incorrect crown catalog canvas")
    try crown.pngData()!.write(to: directory.appendingPathComponent("compiled-crown.png"))
    for (name, width) in [("thin", CGFloat(0.55)), ("medium", CGFloat(0.9)), ("bold", CGFloat(1.25))] {
        for density in [CGFloat(1), 2, 3, 6] {
            let renderer = ImageRenderer(content: CrownOutlineCandidate(outlinePoints: width))
            renderer.scale = density
            guard let bytes = renderer.uiImage?.pngData() else {
                throw NSError(domain: "CrownOutline", code: 1)
            }
            try bytes.write(to: directory.appendingPathComponent("outline-\(name)@\(Int(density))x.png"))
        }
    }
    // Capture the production color treatment before gameplay downsampling.
    for density in [CGFloat(1), 2, 3] {
        let face = Image("path_exit_crown").resizable().interpolation(.high).scaledToFit()
            .frame(width: 96, height: 96)
            .colorEffect(ShaderLibrary.flatExitCrown()).drawingGroup()
        let renderer = ImageRenderer(content: face)
        renderer.scale = density
        guard let bytes = renderer.uiImage?.pngData() else {
            throw NSError(domain: "ExitMarkerProbe", code: 3)
        }
        try bytes.write(to: directory.appendingPathComponent("flat-face@\(Int(density))x.png"))
    }
    let rect = CGRect(x: 0, y: 0, width: 340 * 16 / 9, height: 340)
    ...
    try require(abs(spriteSize - 25.6) < 0.000001, "Incorrect minimum marker size")
    ...
    return try levels.map { level in
        ...
        return ["map": level.mapImageName, "count": expected.count,
                "minimumBoxPoints": spriteSize, "verticalFraction": 1.0, "sourceRollDegrees": 20.0, "correctedRollDegrees": 10.0, "perspectiveRecovery": cos(27.5 * Double.pi / 180) / cos(55 * Double.pi / 180), "positions": positions]
    }
}
