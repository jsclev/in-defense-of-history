"""Readability lab using flat-face renders captured on the physical iPhone."""
from pathlib import Path
from PIL import Image, ImageDraw
import importlib.util,json,math,hashlib
ROOT=Path('/Users/john/projects/td');DATA=ROOT/'in-defense-of-history-data'
REPORT=DATA/'ArtReadability/reports/gold-crown-flat-2026-09-10'
DEVICE=ROOT/'in-defense-of-history/Tools/reports/gold-crown-flat-2026-09-10/device-documents'
...
files=[]
for den in (1,2,3):
 im=Image.open(DEVICE/f'flat-face@{den}x.png').convert('RGBA')
...
old={'id':'previous_half_tilt','label':'Before / raised sidewall','animated':False,'map_height':51.2*655/340,'ground_inset_fraction':.25,'states':{d:{'idle':['crown'],'walk':['crown']} for d in lab.DIRECTIONS},'files':{'crown':[str(DATA/f'ArtReadability/reports/gold-crown-half-tilt-2026-09-10/corrected-diagnostic@{den}x.png') for den in (1,2,3)]}}
new={'id':'flat_crown','label':'After / flat path marking','animated':False,'map_height':51.2*655/340,'ground_inset_fraction':.25,'states':{d:{'idle':['crown'],'walk':['crown']} for d in lab.DIRECTIONS},'files':{'crown':files}}
config['assets']=[old,new]
config['sizing_note']='Both use the unchanged 25.6pt source frame and half-tilt transform. After removes the violet sidewall with the production shader captured on the iPhone before downsampling. 51.2pt padded QA canvas contains transform pixels; it does not enlarge the runtime frame.'
...
draw.text((18,14),'Flat crown / before and after at minimum size',font=lab.font(19),fill='#30343c')
draw.text((18,43),'Same size and angle; raised purple edge removed',font=lab.font(13),fill='#30343c')
old_im=lab.thumbnail(Image.open(old['files']['crown'][2]).convert('RGBA'),51.2)
new_im=lab.thumbnail(Image.open(files[2]).convert('RGBA'),51.2)
for i,(label,im) in enumerate([('Before',old_im),('Flat',new_im)]):
