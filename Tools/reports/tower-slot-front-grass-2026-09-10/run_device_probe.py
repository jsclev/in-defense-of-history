...
BUNDLE_ID = 'com.zippyzen.slot-front-grass-probe'
RUN_ID = str(uuid.uuid4())

def run(*args, **kwargs):
    return subprocess.run(args, check=True, **kwargs)

with tempfile.TemporaryDirectory(prefix='td-slot-front-grass-probe-') as temp_name:
    temp = Path(temp_name)
    root = temp / GAME.name
    root.mkdir()
    ...
    compiled = Path('/private/tmp/td-slot-front-grass-device-build/Build/Products/Debug-iphoneos/Liberty Line.app/Assets.car')
    if compiled.exists():
    ...
    inputs = [GAME / 'Liberty Line/Level Views/LevelTowerSlotsView.swift', GAME / 'Liberty Line/Level Views/LevelMapView.swift']
    ...
    print('PASS: updated front grass blend, availability transition, gameplay captures; temporary probe removed.')
