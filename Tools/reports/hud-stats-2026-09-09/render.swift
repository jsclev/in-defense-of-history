...
struct HudStatsView: View {
    ...
    private static func dimensions(in runtimeCanvas: RuntimeCanvas) -> (panel: StatsPanelLayout, size: CGSize, waveHeight: CGFloat) {
        ...
        let width = panel.lives.valueBox.maxX - panel.lives.icon.minX
            + panel.money.valueBox.maxX - panel.money.icon.minX + 5 * pad
        let waveHeight = panel.waveFontSize * 1.25 + 1.2 * pad
        let height = max(panel.lives.icon.height, panel.money.icon.height) + 3 * pad + waveHeight
        return (panel, CGSize(width: width, height: height), waveHeight)
    }
    ...
    var body: some View {
        ...
        VStack(spacing: metrics.statPlatePadding) {
            ...
            counterText(waveText, fontSize: panel.waveFontSize)
                .frame(width: width, height: layout.waveHeight)
    ...
    private var waveText: String { "Wave \(runner.currentWaveNumber) of \(runner.waveCount)" }
