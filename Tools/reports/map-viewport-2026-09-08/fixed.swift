 ...
 @MainActor static func main() throws {
  for screen in [CGSize(width:852,height:393),CGSize(width:1024,height:768)] {
   for show in [true,false] {
    let stage = LevelViewport(size:screen) {
    ...
    try rep.representation(using:.png,properties:[:])!.write(to:URL(fileURLWithPath:"/tmp/td-map-stability/fixed-\(Int(screen.width))-\(show).png"))
