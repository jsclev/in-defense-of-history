...
BUNDLE_ID = 'com.zippyzen.tower-silhouette-probe'
...
with tempfile.TemporaryDirectory(prefix='td-ranged-probe-') as temp_name:
    ...
    inputs = [GAME / 'Liberty Line/Level Views/LevelMapView.swift', GAME / 'Engine/Models/TowerKind.swift', GAME / 'Engine/Layout/TowerMenuLayout.swift']
    names = ['ranged_tower_level_1','ranged_tower_level_2','ranged_tower_level_3'] + [f'ranged_tower_level_4_branch_{x}' for x in (1,2,3)] + ['tower_menu_ranged_square','hud_misc_sack_blue_frame']
    for name in names:
    ...
    with (REPORT / 'probe-build.log').open('w') as log:
        run('xcodebuild', '-project', str(root / 'InDefenseOfHistory.xcodeproj'), '-scheme', 'Liberty Line',
            ...
            'INFOPLIST_KEY_CFBundleDisplayName=Tower Silhouette Check', 'build', stdout=log, stderr=subprocess.STDOUT)
