...
private func buildHeroRoads() -> HeroRoads {
        ...
        for path in paths {
            ...
            var travelled = 0.0
            // Navigation follows the complete authored road, including exits
            // and bends behind the map's edge foliage. Clipping to a sprite-
            // safe rectangle can split one road into disconnected pieces.
            while true {
                ...
                previous = index
                if travelled >= path.totalLength { break }
                travelled = min(travelled + Self.heroRoadSpacing, path.totalLength)
            }
        }
