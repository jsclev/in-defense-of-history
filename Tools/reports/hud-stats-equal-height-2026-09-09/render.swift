...
struct HudStatsView: View {
    ...
    private static func dimensions(in runtimeCanvas: RuntimeCanvas) -> (panel: StatsPanelLayout, size: CGSize, waveWidth: CGFloat, spacing: CGFloat) {
        ...
        let width = panel.lives.valueBox.maxX - panel.lives.icon.minX
            ...
            + (2 + 2 * Self.leadingPaddingMultiplier) * pad + 2 * spacing + waveWidth
    ...
    var body: some View {
        ...
        HStack(spacing: layout.spacing) {
            counter(icon: "lives_icon_05", value: livesText, row: panel.lives, plateHeight: height)
                .accessibilityLabel("Lives: \(runner.lives)")
            counter(icon: "money_icon_12", value: goldText, row: panel.money, plateHeight: height)
    ...
    @MainActor static func main() throws {
        ...
        for (name, physical, safe) in fixtures {
            ...
            for (state, lives, money, wave, total) in [("start",20,500,1,15),("end",1,9999,15,15),("wide",99,9999,99,99)] {
                ...
                measurements.append(["fixture":name,"state":state,"width":occupied.width,"height":occupied.height,"font":HudMetrics(runtimeCanvas:canvas).moneyTextSize])
            }
        }
