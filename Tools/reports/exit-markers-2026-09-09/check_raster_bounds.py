...
source = Image.open(DATA / 'LibertyLineAssets.xcassets/path_exit_rounded_x.imageset/path_exit_rounded_x@3x.png').convert('RGBA')
    ...
    for density in (1, 3):
        ...
        for point in level['positions']:
            ...
            expected = [cx + (box[0] / source.width - 0.5) * side,
                        cy + (box[1] / source.height - 0.5) * side,
                        cx + (box[2] / source.width - 0.5) * side,
                        cy + (box[3] / source.height - 0.5) * side]
